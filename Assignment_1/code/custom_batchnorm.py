...
class CustomBatchNormAutograd(nn.Module):
  ...
  def forward(self, input):
    ...
    # PUT YOUR CODE HERE  #
    #######################
    [n_batch, n_neurons] = input.size()
    assert self.n_neurons == n_neurons, 'The input shape seems to be incorrect'

    mean = input.mean(dim=0, keepdim=True)
    var = input.var(dim=0, keepdim=True, unbiased=False)
    input_normalised = (input - mean)/torch.sqrt(var + self.eps)
    out = self.gamma*input_normalised + self.beta
    ########################
    # END OF YOUR CODE    #
    #######################
