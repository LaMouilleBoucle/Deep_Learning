...
def accuracy(predictions, targets):
  ...
  # PUT YOUR CODE HERE  #
  #######################
  #correct = sum(targets[np.arange(targets.shape[0]), np.argmax(predictions, axis=1)]) # perhaps a little faster?
...
def train():
  ...
  logloss_module = CrossEntropyModule()
  ...
  for i in range(FLAGS.max_steps):
      ...
      out = mlp.forward(x)
      dout = logloss_module.backward(out, y)
      mlp.backward(dout)
      mlp.update(FLAGS.learning_rate)

      if (i+1) % FLAGS.eval_freq == 0:
          avg_losses += [logloss_module.forward(out, y)]
          accuracies += [accuracy(out,y)]
          out_test = mlp.forward(x_test)
          avg_losses_test += [logloss_module.forward(out_test, y_test)]
          accuracies_test += [accuracy(out_test, y_test)]
